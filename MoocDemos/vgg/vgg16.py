...
class Vgg16():
    ...
    def forward(self, images):
        ...
        rgb_scaled = images * 255.0 # 逐像素乘以255.0, (根据原论文的初始化步骤)
        red, green, blue = tf.split(rgb_scaled,3,3) # 从GRB转换色彩通道到BGR, 也可使用cv中的GRBtoBGR
        # 加入断言, 判断每个操作之后的维度变化是否和预期一致
        assert red.get_shape().as_list()[1:] == [224, 224, 1]
        # 省略

        # 逐样本减去每隔通道的像素平均, 这种操作可以移除图像的平均亮度值, 该方法常用在灰度图像上.
        bgr = tf.concat([
            blue - VGG_MEAN[0],
            green - VGG_MEAN[1],
            red - VGG_MEAN[2]],3)
        ...
        # 接下来构建VGG的16层网络,包含五段卷积, 三层全连接, 并逐层根据命名空间读取网络参数

        # 第一段卷积, 含有两个卷基层, 后面接最大池化层, 用来缩小图片尺寸
        self.conv1_1 = self.conv_layer(bgr, "conv1_1")

        # 传入命名空间的name, 来获取该层卷积核和偏置, 并做卷积运算, 最后返回经过激活函数后的值
        ...
        # 第六层全连接
        self.fc6 = self.fc_layer(self.pool5, "fc6") # 根据命名空间name 做加权求和运算
    ...
    def fc_layer(self, x, name):
        ...
        with tf.variable_scope(name): 
            ...
            result = tf.nn.bias_add(tf.matmul(x, w), b) # 对该输入层做加权求和, 再加上偏置
            return result
    
    def get_fc_weight(self, name):
        """定义获取权重的函数
        根据命名空间name从参数字典中取到相应的权重.
        """
